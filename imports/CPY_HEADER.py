...
all , any  = all **OP_TO_UNARY_ , any **OP_TO_UNARY_ 
...
FILTER_POS_  = (<$$>(list(filter( ARGS_ [0],* ARGS_ [1:])) if  len ( ARGS_ )>1 else  (<$* ARGS_ ,f= ARGS_ [0]$>list(filter(f,* ARGS_ ))) **OP_TO_UNARY_ )) **OP_TO_BNARY_ 
FILTER_NEG_  = (<$$>(list(filter(<$x,f= ARGS_ [0]$> not f(x),* ARGS_ [1:])) if  len ( ARGS_ )>1 else  (<$* ARGS_ ,f= ARGS_ [0]$>list(filter(<$x$> not f(x),* ARGS_ ))) **OP_TO_UNARY_ )) **OP_TO_BNARY_ 
FOLD_  = (<$$>(reduce(* ARGS_ ) if  len ( ARGS_ )>1 else  (<$* ARGS_ ,f= ARGS_ [0]$>reduce(f,* ARGS_ )) **OP_TO_UNARY_ )) **OP_TO_BNARY_ 
sum  = (x$>reduce(<$x,y$>x+y,(x:=list(x)),*([ ARGS_ [0] if ARGS_  else 0] if  not x else []))) **OP_TO_BNARY_ 
...
print  = (<$$>print_(* ARGS_ ,** KWARGS_ ) or ( ARGS_ [0] if ARGS_ )) **OP_TO_UNARY_ 
